		...
		try {
			while (true) {
				...
				case "text to room": {
					...
					MessageData newMessage = new MessageData(thisClient.userName, "text", content);
				...
				case "deleteMessage": {
					try {
						String roomIdStr = thisClient.receiver.readLine();
						String messageIndexStr = thisClient.receiver.readLine();
						...
						if (roomIdStr != null && messageIndexStr != null) {
							...
							if (room != null && messageIndex >= 0 && messageIndex < room.messages.size()) {
								MessageData message = room.messages.get(messageIndex);
								// Chỉ xóa tin nhắn nếu người gửi là người dùng hiện tại
								if (message.whoSend.equals(thisClient.userName)) {
									room.messages.remove(messageIndex);
									// Gửi thông báo xóa tin nhắn cho tất cả người dùng trong room
									for (String user : room.users) {
										Client currentClient = Client.findClient(Main.socketController.connectedClient, user);
										if (currentClient != null) {
											currentClient.sender.write("messageDeleted");
											currentClient.sender.newLine();
											currentClient.sender.write("" + roomId);
											currentClient.sender.newLine();
											currentClient.sender.write("" + messageIndex);
											currentClient.sender.flush();
										}
									}
								}
				...
				case "getRoomMessages": {
					...
					Room room = Room.findRoom(Main.socketController.allRooms, roomId);
					if (room != null) {
						// Gửi số lượng tin nhắn chưa bị xóa
						int visibleMessageCount = 0;
						for (MessageData message : room.messages) {
							if (!message.deletedBy.contains(thisClient.userName)) {
								visibleMessageCount++;
							}
						}
						thisClient.sender.write("" + visibleMessageCount);
						thisClient.sender.newLine();
						thisClient.sender.flush();

						// Gửi từng tin nhắn chưa bị xóa
						for (MessageData message : room.messages) {
							if (!message.deletedBy.contains(thisClient.userName)) {
								thisClient.sender.write("text from user to room");
								thisClient.sender.newLine();
								thisClient.sender.write(message.whoSend);
								thisClient.sender.newLine();
								thisClient.sender.write("" + roomId);
								thisClient.sender.newLine();
								thisClient.sender.write(message.content);
								thisClient.sender.write('\0');
								thisClient.sender.flush();
							}
						}
					}
					...
					try {
						...
						if (roomIdStr != null) {
							...
							if (room != null) {
								...
								// Thông báo cho tất cả người dùng trong room
								for (String user : room.users) {
									Client currentClient = Client.findClient(Main.socketController.connectedClient, user);
									if (currentClient != null) {
										currentClient.sender.write("messagesCleared");
										currentClient.sender.newLine();
										currentClient.sender.write("" + roomId);
										currentClient.sender.newLine();
										currentClient.sender.flush();
